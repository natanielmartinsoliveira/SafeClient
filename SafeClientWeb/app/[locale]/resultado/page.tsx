  ...
  return (
    <main className="min-h-screen flex flex-col items-center" style={{ background: '#7B52B8' }}>
      ...
      <div className="w-full max-w-2xl px-6 py-6 flex flex-col gap-5" style={{ marginTop: -1 }}>
        {error && (
          <div className="rounded-2xl p-6 flex flex-col items-center gap-3" style={{ background: 'rgba(255,255,255,0.14)' }}>
            <span className="text-4xl">⚠️</span>
            <p className="text-white text-sm text-center">{error}</p>
            <Link href="/" className="px-6 py-2 rounded-full text-sm font-bold text-white" style={{ background: 'rgba(255,255,255,0.2)' }}>
              {t('retry')}
            </Link>
          </div>
        )}
        ...
        {result?.found && (() => {
          ...
          return (
            <>
              {/* Risk banner */}
              <div className="rounded-2xl p-5 flex items-center gap-4" style={{ background: 'rgba(255,255,255,0.14)', border: '1px solid rgba(255,255,255,0.2)' }}>
                <div className="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 text-2xl" style={{ background: risk.bg }}>
                  {risk.emoji}
                </div>
                <div>
                  <p className="text-xl font-bold" style={{ color: risk.color }}>{risk.label}</p>
                  <p className="text-sm" style={{ color: 'rgba(255,255,255,0.7)' }}>
                    {result.reportCount} {result.reportCount === 1 ? t('warningOne') : t('warningMany')}
                  </p>
                </div>
              </div>
                ...
                {(result.flags ?? []).map((flag, i) => (
                  <div key={flag}>
                    <div className="flex items-center gap-3 px-5 py-4">
                      <div className="w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0" style={{ border: '1.5px solid rgba(255,255,255,0.5)' }}>
        ...
        {result && !result.found && (
          <div className="flex flex-col items-center gap-4 py-12">
            ...
            <p className="text-sm text-center" style={{ color: 'rgba(255,255,255,0.7)' }}>{t('noReportsDesc1')}</p>
            <p className="text-sm text-center" style={{ color: 'rgba(255,255,255,0.7)' }}>{t('noReportsDesc2')}</p>
          </div>
        )}
