...
const [report, setReport] = useState<AdminReport | null>(null);
  ...
  if (loading || !report) return <p style={{ color: '#9887B8' }}>Carregando...</p>;

  return (
    <div className="flex flex-col gap-6 max-w-lg">
      <div className="flex items-center gap-4">
        <button onClick={() => router.back()} className="text-sm" style={{ color: '#9887B8' }}>
          ← Voltar
        </button>
      ...
      >
        <div className="text-xs" style={{ color: '#9887B8' }}>
          <p>
            Tipo: <strong style={{ color: '#2E1B6E' }}>{report.contactType}</strong>
          </p>
          <p>
            Criado em:{' '}
            <strong style={{ color: '#2E1B6E' }}>
              {new Date(report.createdAt).toLocaleString('pt-BR')}
            </strong>
          </p>
          {report.userEmail && (
            <p>
              Autor: <strong style={{ color: '#2E1B6E' }}>{report.userEmail}</strong>
            </p>
          )}
        </div>
        ...
        <label className="flex items-center gap-2 cursor-pointer">
          <input
            ...
            className="w-4 h-4 accent-purple-600"
          />
          <span className="text-sm font-medium" style={{ color: '#2E1B6E' }}>
            Relato ativo
          </span>
        </label>
