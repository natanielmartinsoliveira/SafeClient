    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-black" style={{ color: '#2E1B6E' }}>
          Relatos <span className="text-base font-normal" style={{ color: '#9887B8' }}>({total})</span>
        </h1>
        <select
          value={activeFilter}
        ...
        <table className="w-full text-sm" style={{ background: '#FFFFFF' }}>
          <thead>
            <tr style={{ borderBottom: '1px solid #E0D8F4', background: '#F9F7FF' }}>
              <th className="text-left px-4 py-3 font-semibold" style={{ color: '#6B5B9E' }}>Tipo</th>
              <th className="text-left px-4 py-3 font-semibold" style={{ color: '#6B5B9E' }}>Flags</th>
              <th className="text-left px-4 py-3 font-semibold" style={{ color: '#6B5B9E' }}>Usuário</th>
              <th className="text-left px-4 py-3 font-semibold" style={{ color: '#6B5B9E' }}>Status</th>
              <th className="text-left px-4 py-3 font-semibold" style={{ color: '#6B5B9E' }}>Data</th>
              <th className="px-4 py-3" />
            </tr>
          </thead>
          <tbody>
            {loading ? (
              <tr><td colSpan={6} className="text-center py-8" style={{ color: '#9887B8' }}>Carregando...</td></tr>
            ) : reports.length === 0 ? (
              <tr><td colSpan={6} className="text-center py-8" style={{ color: '#9887B8' }}>Nenhum relato encontrado.</td></tr>
            ) : reports.map((r, i) => (
              <tr key={r.id} style={{ borderTop: i > 0 ? '1px solid #F0ECFF' : undefined }}>
                <td className="px-4 py-3 font-medium" style={{ color: '#2E1B6E' }}>{r.contactType}</td>
                ...
                <td className="px-4 py-3 flex gap-2 justify-end">
                  ...
                  {r.active && (
                    <button onClick={() => softDelete(r.id)}
                      className="text-xs px-3 py-1 rounded-lg font-medium"
